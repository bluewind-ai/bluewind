...
class BaseModel(CloneMixin, models.Model):
    ...
    def clone_workspace_related(cls, old_workspace, new_workspace):
        logger.info(f"Starting to clone {cls.__name__} objects from workspace {old_workspace} to {new_workspace}")
        objects = cls.objects.filter(workspace=old_workspace)
        logger.info(f"Found {objects.count()} {cls.__name__} objects to clone")
        for obj in objects:
            logger.info(f"Cloning {cls.__name__} object with id: {obj.id}, public_id: {obj.public_id}")
            cloned_obj = obj.make_clone(new_workspace=new_workspace)
            logger.info(f"Cloned {cls.__name__} object. Original id: {obj.id}, Clone id: {cloned_obj.id}")
        logger.info(f"Finished cloning {cls.__name__} objects for workspace {old_workspace}")