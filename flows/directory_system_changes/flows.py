...
def directory_system_changes(file_change):
    logger.debug(
        f"Directory change ({file_change.change_type}) detected for {file_change.source_path}"
    )

    user = file_change.workspace.user
    ...
    if file_change.change_type == "created":
        ...
        File.objects.filter(
            workspace=file_change.workspace, path__startswith=file_change.source_path
        ).delete()
    elif file_change.change_type == "modified":
        # Handle directory modification (e.g., rename)
        # This might involve updating the paths of all files within the directory
        old_path = file_change.source_path
        new_path = file_change.destination_path  # Assuming this exists in your model
        if new_path:
            files_to_update = File.objects.filter(
                workspace=file_change.workspace, path__startswith=old_path
            )
