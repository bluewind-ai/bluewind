...
#         instance = instance or self.create()
#         return reverse(
#             "admin:{app}_{model}_delete".format(
#                 app=self.model._meta.app_label, model=self.model._meta.model_name
#             ),
#             args=[instance.pk],
#         )
...
#         return {
#             x: y
#             for x, y in filter(lambda x: x[1], model_to_dict(instance).items())
...
#                                 f"Sample{field.label[:max_length-6]}"
#                             )
#                         else:
...
#                 if self.add_status_code != 200
#                 else "Skip add is enabled"
#             )
...
#             raise SkipTest(
#                 "Required status code != 200"
#                 if self.change_status_code != 200
