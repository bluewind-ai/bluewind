...
class SessionStoreTest(TestCase):
    ...
    def test_save(self):
        ...
        self.assertAlmostEqual(now(), session.last_activity, delta=timedelta(seconds=5))

    def test_load_unmodified(self):
        self.store[auth.SESSION_KEY] = 1
        self.store.save()
        store2 = SessionStore(
            session_key=self.store.session_key, user_agent="Python/2.7", ip="127.0.0.1"
        )
        store2.load()
    ...
    def test_load_modified(self):
        ...
        self.store.save()
        store2 = SessionStore(
            session_key=self.store.session_key, user_agent="Python/3.3", ip="8.8.8.8"
        )
        store2.load()
    ...
    def test_duplicate_create(self):
        s1 = SessionStore(
            session_key="DUPLICATE", user_agent="Python/2.7", ip="127.0.0.1"
        )
        s1.create()
        s2 = SessionStore(
            session_key="DUPLICATE", user_agent="Python/2.7", ip="127.0.0.1"
        ...
        s3 = SessionStore(
            session_key=s1.session_key, user_agent="Python/2.7", ip="127.0.0.1"
        )
