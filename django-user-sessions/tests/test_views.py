...
class ViewsTest(TestCase):
    ...
    def test_list(self):
        ...
        with self.assertWarnsRegex(
            UserWarning, r"The address 127\.0\.0\.1 is not in the database"
        ):
    ...
    def test_delete(self):
        ...
        response = self.client.post(
            reverse("user_sessions:session_delete", args=[session_key])
        )
        self.assertRedirects(response, "/")

    def test_delete_all_other(self):
        self.user.session_set.create(
            ip="127.0.0.1", expire_date=now() + timedelta(days=1)
        )
    ...
    def test_delete_some_other(self):
        other = self.user.session_set.create(
            session_key="OTHER", ip="127.0.0.1", expire_date=now() + timedelta(days=1)
        )
        self.assertEqual(self.user.session_set.count(), 2)
        response = self.client.post(
            reverse("user_sessions:session_delete", args=[other.session_key])
        )
        with self.assertWarnsRegex(
            UserWarning, r"The address 127\.0\.0\.1 is not in the database"
        ):
